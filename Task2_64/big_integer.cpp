...
big_integer::big_integer(std::string const& s)
{
    ...
    {
        uint32 mod = long_mod_short(tmp, BASE, 10);
...
void big_integer::divide(std::vector<uint32> &res, std::vector<uint32> const &a, std::vector<uint32> const &b)
{
    ...
    {
        uint64 tmp = (uint64) ((uint64)(n + j < u.size() ? u[n + j] : 0)* BASE + u[n + j - 1]) / v[n - 1];
        res[j] = std::min(tmp, BASE - 1);
        std::vector <uint32> temp_vector;
        multiply(temp_vector, pow_base(j), v);
